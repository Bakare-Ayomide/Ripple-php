import { Settings, Grid3X3, Bookmark, Flame, LogOut, Loader2, Camera, Upload, EyeOff } from "lucide-react";
  ...
  return (
    <div className="max-w-[900px] mx-auto px-3 pt-4 lg:pt-6">
      ...
      <div className="bg-card rounded-3xl p-5 border border-border shadow-card mb-4">
        <h3 className="text-sm font-display font-extrabold text-foreground mb-1 flex items-center gap-1.5">
          <EyeOff className="w-4 h-4 text-[#ec4899]" /> Wave Propagation Privacy
        </h3>
        <p className="text-xs text-muted-foreground font-medium mb-4">
          Control how your redistributions, reposts, and quote reflections are tracked on the D3 interactive visualization tree.
        </p>
        <div className="flex items-center justify-between p-3.5 rounded-xl bg-[#011413]/40 border border-[#0d5c56]/20">
          <div>
            <p className="text-xs font-bold text-foreground">Hide Propagation Details</p>
            <p className="text-[10px] text-muted-foreground mt-0.5">Your shares will appear as anonymous nodes to safeguard your identity.</p>
          </div>
          <button
            onClick={async () => {
              try {
                await updateProfile.mutateAsync({
                  hidePropagationDetails: !(profile as any)?.hide_propagation_details
                });
            ...
            className={`w-11 h-6 rounded-full p-1 transition-colors duration-200 outline-none flex items-center ${
              (profile as any)?.hide_propagation_details ? "bg-[#10b981]" : "bg-secondary"
            }`}
          >
            <div
              className={`w-4 h-4 rounded-full bg-white transition-transform duration-200 transform ${
                (profile as any)?.hide_propagation_details ? "translate-x-5" : "translate-x-0"
              }`}
            />
          </button>
        </div>
