...
const CreatePostModal = ({ open, onClose }: Props) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const now = new Date();
      let scheduledForStr: string | null = null;
      let statusVal = "published";
      let isPublishedVal = true;

      if (isScheduled) {
        if (!scheduledDate) {
          return toast.error("Please specify a schedule date and time");
        }
        ...
        const maxDays = systemSettings?.max_scheduling_duration_days ?? 365;
        const maxTime = new Date(now.getTime() + maxDays * 24 * 60 * 60 * 1000);
        if (schedTime > maxTime) {
          return toast.error(`Scheduled date cannot exceed maximum limit of ${maxDays} days`);
        }

        scheduledForStr = schedTime.toISOString();
        statusVal = "scheduled";
        isPublishedVal = false;
      }

      let unlocksAtStr: string | null = null;
      if (isTimeCapsule) {
        if (!unlocksDate) {
          return toast.error("Please specify when the time capsule should unlock");
        }
        const unlockTime = new Date(unlocksDate);
        if (unlockTime <= now) {
          return toast.error("Unlock date must be in the future");
        }
        unlocksAtStr = unlockTime.toISOString();
      }
      ...
      await createPost.mutateAsync({
        ...
        unlocks_at: unlocksAtStr,
        is_recurring: isScheduled && isRecurring,
        recurrence_interval: isScheduled && isRecurring ? recurrenceInterval : null,
        status: statusVal,
        is_published: isPublishedVal,
      });

      toast.success(isScheduled ? "Scheduled drop queued!" : "Drop streamed!");
      setUploadPct(0);
      setCaption("");
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            {/* Scheduling and Time Capsule Options */}
            <div className="px-5 pb-4 pt-3 border-t border-white/5 space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-display font-bold text-foreground">Schedule Drop</p>
                  <p className="text-xs text-muted-foreground">Release automatically at a future time</p>
                </div>
                <input
                  type="checkbox"
                  checked={isScheduled}
                  onChange={(e) => {
                    ...
                    if (e.target.checked) setIsTimeCapsule(false);
                  }}
                  className="w-4 h-4 rounded border-white/10 bg-secondary text-primary focus:ring-primary focus:ring-offset-0"
                />
              </div>

              {isScheduled && (
                <div className="space-y-3.5 pl-4 border-l-2 border-primary/40 animate-fade-in">
                  <div>
                    <label className="text-xs font-display font-bold text-muted-foreground block mb-1">Publication Date & Time</label>
                    <input
                      type="datetime-local"
                      value={scheduledDate}
                      onChange={(e) => setScheduledDate(e.target.value)}
                      className="bg-secondary/85 rounded-xl px-3 py-2 text-xs text-foreground outline-none focus:ring-2 focus:ring-primary w-full border border-white/5"
                    />
                    {systemSettings?.max_scheduling_duration_days && (
                      <p className="text-[10px] text-muted-foreground mt-1">
                        * Maximum limit: {systemSettings.max_scheduling_duration_days} days in the future
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-display font-bold text-foreground">Recurring Schedule</p>
                      <p className="text-[10px] text-muted-foreground">Repeat publication periodically</p>
                    </div>
                    <input
                      type="checkbox"
                      checked={isRecurring}
                  ...
                  {isRecurring && (
                    <div className="animate-fade-in">
                      <label className="text-xs font-display font-bold text-muted-foreground block mb-1">Recurrence Interval</label>
                      <select
                        value={recurrenceInterval}
                        onChange={(e) => setRecurrenceInterval(e.target.value)}
                        className="bg-secondary/85 rounded-xl px-3 py-2 text-xs text-foreground outline-none focus:ring-2 focus:ring-primary w-full border border-white/5 cursor-pointer"
                      >
              ...
              <div className="flex items-center justify-between border-t border-white/5 pt-3">
                <div>
                  <p className="text-sm font-display font-bold text-foreground">Time Capsule (Private Locked)</p>
                  <p className="text-xs text-muted-foreground">Keeps content private and locked until a specific future date</p>
                </div>
                <input
                  type="checkbox"
              ...
              {isTimeCapsule && (
                <div className="space-y-3 pl-4 border-l-2 border-accent/40 animate-fade-in">
                  <div>
                    <label className="text-xs font-display font-bold text-muted-foreground block mb-1">Unlock Date & Time</label>
                    <input
                      type="datetime-local"
                      value={unlocksDate}
                      onChange={(e) => setUnlocksDate(e.target.value)}
                      className="bg-secondary/85 rounded-xl px-3 py-2 text-xs text-foreground outline-none focus:ring-2 focus:ring-accent w-full border border-white/5"
                    />
                  </div>
                </div>
              )}
